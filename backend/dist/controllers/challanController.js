...
exports.cancelChallan = exports.confirmChallan = exports.createChallan = exports.getChallanById = exports.listChallans = void 0;
...
const getChallanById = async (req, res) => {
    try {
        await ensureChallanTables();
        const challanResult = await database_1.pool.query('SELECT * FROM sales_challans WHERE id = $1', [req.params.id]);
        if (challanResult.rows.length === 0) {
            return res.status(404).json({ message: 'Challan not found', status: 'error' });
        }
        const itemsResult = await database_1.pool.query('SELECT * FROM challan_items WHERE challan_id = $1 ORDER BY id ASC', [req.params.id]);
...
const createChallan = async (req, res) => {
    try {
        ...
        try {
            ...
            const challanResult = await client.query(`INSERT INTO sales_challans (challan_number, customer_id, status, created_by, notes)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`, [challanNumber, Number(customer_id), challanStatus, req.user?.email || 'system', notes || null]);
            ...
            for (const item of items) {
                await client.query(`INSERT INTO challan_items (challan_id, product_id, product_name_snapshot, sku_snapshot, quantity, unit_price_snapshot)
           VALUES ($1, $2, $3, $4, $5, $6)`, [item.challan_id, item.product_id, item.product_name_snapshot, item.sku_snapshot, item.quantity, item.unit_price_snapshot]);
            }
...
const confirmChallan = async (req, res) => {
    try {
        await ensureChallanTables();
        const { id } = req.params;
        const challanResult = await database_1.pool.query('SELECT * FROM sales_challans WHERE id = $1', [id]);
        if (challanResult.rows.length === 0) {
            return res.status(404).json({ message: 'Challan not found', status: 'error' });
        }
        ...
        const itemsResult = await database_1.pool.query('SELECT * FROM challan_items WHERE challan_id = $1', [id]);
        ...
        try {
            ...
            for (const entry of inventoryChecks) {
                await client.query('UPDATE inventory SET quantity = quantity - $1, updated_at = NOW() WHERE product_id = $2', [entry.quantity, entry.product_id]);
                await client.query(`INSERT INTO stock_movements (product_id, movement_type, quantity, reason, created_by)
           VALUES ($1, 'OUT', $2, 'Challan confirmation', $3)`, [entry.product_id, entry.quantity, req.user?.id ?? null]);
            }
            const updatedChallan = await client.query('UPDATE sales_challans SET status = $1 WHERE id = $2 RETURNING *', ['CONFIRMED', id]);
            await client.query('COMMIT');
...
const cancelChallan = async (req, res) => {
    try {
        ...
        const updated = await database_1.pool.query('UPDATE sales_challans SET status = $1 WHERE id = $2 RETURNING *', ['CANCELLED', req.params.id]);
